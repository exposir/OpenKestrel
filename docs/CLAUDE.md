...
- 主要功能：沉淀项目文档规范，负责设计说明、约束定义与知识索引，对应目录「docs/ - 双轨同构文档系统」。
- 核心文件：`directory.zh.md`（模块文档与规范）。
- 实现原理：采用单入口实现：由 `directory.zh.md` 直接承载核心逻辑并对外提供可调用能力。
- 相关文件：上游规范 [../CLAUDE.md](./../CLAUDE.md)；下游模块 `intent/`、`logic/`；同级协作见本文件“成员清单”。
## 成员清单
