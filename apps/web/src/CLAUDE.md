...
- 主要功能：提供核心业务实现能力，负责领域逻辑、依赖装配与外部适配，对应目录「src/ - 核心引擎」。
- 核心文件：`audit/`（模块实现）、`auth/`（模块实现）、`di/`（模块实现）、`env/`（模块实现）。
- 实现原理：采用“入口 -> 处理 -> 输出”链路：由 `audit/` 接入调用，再由 `auth/` 与 `di/` 完成主要处理，最后对上层暴露稳定结果。
- 相关文件：上游规范 [../CLAUDE.md](./../CLAUDE.md)；下游模块 `audit/`、`auth/`、`di/`、`env/`；同级协作见本文件“成员清单”。
## 成员清单
