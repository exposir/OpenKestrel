...
### layer_phenomenal（现象层）
- 职责：捕捉错误痕迹、日志碎片、堆栈回声；理解困惑表象、痛点症状；记录可复现路径。
- 输入：“程序崩溃了”。
- 收集：错误类型、时机节点、触发条件。
...
- 脆弱性：一处修改导致无关部分损坏。
- 晦涩性：代码意图不明。
- 结构混乱数据泥团：多个数据项总一起出现，应组合为对象。
- 不必要复杂：过度设计系统臃肿难懂。
- 强制要求：识别代码坏味道立即询问是否优化并给出改进建议，无论任何情况。

### architecture_documentation（架构文档）
...
- `# {模块名}/`
- `> L2 | 父级: {父路径}/CLAUDE.md`
- 模块功能综述：用 4-8 句“说人话”写清本目录的功能目标、核心实现原理、输入/输出与不负责范围；读者应能在 30 秒内看懂该模块做什么、怎么做；禁止空话模板句、纯术语堆砌、仅重复目录名、只列文件不解释协作实现
- 成员清单 `[{文件}.{ext}](./{文件}.{ext})`：`{职责}，{技术细节}，{关键参数}`
- 法则：功能可读·原理可解释·表达清晰可验证·避免空泛表述·成员完整·支持跳转链接·一行一文件·父级链接·技术词前置
- `[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md`
...
法则：INPUT 说清依赖什么·OUTPUT 说清提供什么·POS 说清自己是谁。发现业务文件缺少 L3 头部，立即添加，阻塞级优先。
